...
import api from "../api";
import { fetchReturnPrediction } from "../api";
...
export default function Portfolio() {
  ...
  const runMlPrediction = async () => {
    ...
    const result = await fetchReturnPrediction(ticker);

    if (result?.predicted_return !== undefined) {
      setMlPrediction(result.predicted_return);
    }
  ...
  return (
    <div className="container">
      ...
      <div className="card" style={{ marginTop: 16 }}>
        <h3>ML Market Insight</h3>
        <p className="small">
          Short-term return estimate based on recent market behaviour.
        </p>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <input
            ...
            style={{ padding: "6px" }}
          />
          <button onClick={runMlPrediction} disabled={mlLoading}>
            {mlLoading ? "Running model..." : "Run ML"}
          </button>
        </div>
        ...
        {mlPrediction !== null && (
          <p style={{ marginTop: 8 }}>
            ...
            <strong> {(mlPrediction * 100).toFixed(2)}%</strong>
          </p>
        )}
      </div>
